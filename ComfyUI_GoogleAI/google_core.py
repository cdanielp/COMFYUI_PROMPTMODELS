"""
google_video_node.py - Nodos de Video para ComfyUI (V2.1 + Audio)
==========================================================
...
V2.1 Cambios:
  ...
  - ¡NUEVO! Extracción de AUDIO nativo de Veo 3.1 expuesto a ComfyUI.
...
class GoogleAI_VideoGenerator:
    ...
    async def generate_video(self, prompt, model, video_preset, duration_seconds,
        ...
        try:
            ...
            logger.info(
                f"[VideoGenerator] ✅ {video_tensor.shape[0]} frames @ 24 FPS | Audio OK | {cost_str}"
            )
            logger.info(
...
class GoogleAI_VideoInterpolation:
    ...
    async def interpolate(self, first_frame, last_frame, prompt, model,
        ...
        try:
            ...
            audio_dict = GoogleAICore.video_bytes_to_audio(video_bytes)
            logger.info(
                f"[VideoInterpolation] ✅ {video_tensor.shape[0]} frames @ 24 FPS | Audio OK | {cost_str}"
            )
            logger.info(
                "[Veo 3.1] ⚠️ IMPORTANTE: El video tiene un estándar de 24 FPS. "
                "Configura tu VHS Video Combine a 24 FPS."
...
class GoogleAI_VideoStoryboard:
    ...
    async def generate_storyboard(self, prompt, model, video_preset, duration_seconds,
        ...
        try:
            ...
            audio_dict = GoogleAICore.video_bytes_to_audio(video_bytes)
            logger.info(
                f"[Storyboard] ✅ {video_tensor.shape[0]} frames @ 24 FPS | Audio OK | {cost_str}"
            )
            logger.info(
                "[Veo 3.1] ⚠️ IMPORTANTE: El video tiene un estándar de 24 FPS. "
                "Configura tu VHS Video Combine a 24 FPS."
